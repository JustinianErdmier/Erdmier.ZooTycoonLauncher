...
/// <summary>
///     Implementation of <see cref="IInstallationLocator" /> that walks (in order): the persisted last-known directory, the two hard-coded Program Files paths, then eight
///     registry value-name variants under <c>HKLM\SOFTWARE\Microsoft\Microsoft Games\Zoo Tycoon\1.0</c>. The first directory containing <c>zoo.exe</c> wins.
/// </summary>
public sealed class InstallationLocator : IInstallationLocator
{
    ...
    private static readonly string[] HardCodedProgramFilesPaths = [@"C:\Program Files\Microsoft Games\Zoo Tycoon", @"C:\Program Files (x86)\Microsoft Games\Zoo Tycoon"];

    private static readonly string[] RegistryValueNames = ["InstallPath", "InstallDir", "InstallLocation", "Path", "GameDir", "GamePath", "Install_Dir", "Install_Path"];
    ...
    public Task<LocatedDirectory> LocateAsync(string? persistedLastKnownPath, CancellationToken cancellationToken)
    {
        ...
        if (TryProbe(source: "Persisted last-known", persistedLastKnownPath, trail, out string? hit))
        {
        ...
        {
            if (TryProbe(candidate, candidate, trail, out hit))
            {
                return Task.FromResult(new LocatedDirectory(hit, trail));
            }
        }
        ...
        foreach (string valueName in RegistryValueNames)
        {
            ...
            if (TryProbe($"HKLM\\{ZooKeyPath}\\{valueName}", raw, trail, out hit))
            {
                return Task.FromResult(new LocatedDirectory(hit, trail));
            }
        }
    ...
    private bool TryProbe(string source, string? candidate, List<LocationProbeAttempt> trail, out string? hit)
    {
        ...
        {
            trail.Add(new LocationProbeAttempt(source, CandidatePath: null, LocationProbeFailure.NoValue));
            hit = null;
