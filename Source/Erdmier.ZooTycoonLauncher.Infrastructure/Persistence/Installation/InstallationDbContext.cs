...
/// <summary>EF Core context for a per-installation database (<c>{installationId}.db</c>) — owns the <c>Snapshots</c> and <c>IniValues</c> tables.</summary>
public sealed class InstallationDbContext : DbContext
{
    ...
    public InstallationDbContext(DbContextOptions<InstallationDbContext> options)
        : base(options)
    { }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
        => modelBuilder.ApplyConfigurationsFromAssembly(typeof(InstallationDbContext).Assembly,
                                                        static t => t.Namespace?.StartsWith(value: "Erdmier.ZooTycoonLauncher.Infrastructure.Persistence.Installation",
