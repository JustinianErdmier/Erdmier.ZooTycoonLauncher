...
///     semantics a user double-clicking the file would get (PATH lookup, manifests, side-by-side assemblies). <c> WorkingDirectory </c> is set to the directory of
///     <c> zoo.exe </c> so the game finds <c> zoo.ini </c> and its asset folders via its expected relative-path lookups.
/// </remarks>
public sealed class LauncherService : ILauncherService
{
    ...
    public Task<LaunchResult> LaunchAsync(string exePath)
    {
        ...
        {
            return Task.FromResult(new LaunchResult(Success: false, ErrorMessage: $"Game executable not found at \"{exePath}\"."));
        }
        ...
        try
        {
            ...
            // Process.Start returns null only when UseShellExecute opens the file with the registered handler and no new process is created — not applicable for an .exe, but defend anyway.
        ...
        {
            return Task.FromResult(new LaunchResult(Success: false, ErrorMessage: ex.Message));
        }
    }
}
