...
> one-click launch. Avalonia 11 + Classic.Avalonia, .NET 10, C# 13.
...
A working, single-project prototype existed at the start of this rewrite. That build (the *Ref* assembly) has been retained in the repository under  
`References/Erdmier.ZooTycoonLauncherRef/` for read-only inspiration; nothing in the new launcher is copy-pasted from it. The Ref build informed the design where its choices
proved  
sound (the line-preserving INI tokeniser, the `IFileSystem`/`IRegistryReader` abstractions, the `ZooIniDefaults` registry pattern, the `ViewLocator` convention); each is  
reimplemented from scratch against the new layered architecture and is referenced inline where applicable.
...
10. **INI Config tab** that hosts either the editor or a placeholder — Loading, No INI, or could-not-be-read — based on the installation's state, with dedicated recovery
    actions planned for the No INI and could-not-be-read placeholders. A missing `zoo.ini` resolves the main window to **Cannot Play** with the Launch Game button disabled —
    the launcher will not start the game without a parsed INI.
11. **Scenarios section** within the INI tab with verified key→scenario→tier mapping (Phase 0 research) rendered as a checkbox grid.
...
A separate `Erdmier.ZooTycoonLauncherRef.slnx` opens the Ref assembly in isolation for visual lookup; the main `Erdmier.ZooTycoonLauncher.slnx` keeps the Ref project included so
a  
single solution opens both worlds.

The root namespace for every new project is `Erdmier.ZooTycoonLauncher`. Production projects live under `Source/`; test projects under `Tests/`; the Ref assembly under  
`References/`. Test projects follow `<Project/Assembly Name>.Tests.<Category>` where the category is `Unit`, `Integration`, `Architecture` (e.g.
`Erdmier.ZooTycoonLauncher.Domain.Tests.Unit`); solution-spanning architecture tests attach to the root name (`Erdmier.ZooTycoonLauncher.Tests.Architecture`).
...
---  
flowchart TB  
    Desktop["Erdmier.ZooTycoonLauncher.Desktop<br>(Avalonia + MVVM)"] --> App["Erdmier.ZooTycoonLauncher.Application<br>(slices, CQRS, abstractions)"]  
    App --> Domain["Erdmier.ZooTycoonLauncher.Domain<br>(POCO entities, smart enums)"]  
    Infra["Erdmier.ZooTycoonLauncher.Infrastructure<br>(EF Core, INI, P/Invoke)"] --> App & Domain  
...
There is no legacy migration: greenfield rewrite, no users on the prior `launcher.config` JSON file to bring forward. The Ref assembly continues to read/write its old config
under  
`%APPDATA%\ZooTycoonLauncher\launcher.config` and the two coexist without crosstalk.
...
stateDiagram
  direction LR
  [*] --> lfzt
  lfzt --> rp:Launcher.db migrated, settings loaded
  rp --> vd:pref = DefaultInstallation and DefaultId is set
  rp --> al:pref = DefaultInstallation and DefaultId is null and no rows exist
  rp --> pd:pref = DefaultInstallation and DefaultId is null and rows exist
  rp --> ogi:pref = NoInstallation
  rp --> rp:Last Played or Last Opened but no candidate (fall back to DefaultInstallation)
  ...
  aid --> pi:user registers a valid installation, installation persisted
  aid --> ngif:user cancelled
  vd --> pi:has zoo.exe
  vd --> cp:missing zoo.exe
  vlp --> pi:has zoo.exe
  vlp --> cp:missing zoo.exe
  vlo --> pi:has zoo.exe
  vlo --> cp:missing zoo.exe
  pi --> rtp:parse OK (Current updated if drift detected)
  pi --> cp:HasIni = false (INI tab shows the No INI placeholder)
  pi --> cp:read or snapshot error (INI tab shows the could-not-be-read placeholder)
  rtp --> [*]
  cp --> [*]
  ogi --> [*]
  lfzt:Looking For Zoo Tycoon
  rp:Resolving Preference
  vd:Verify Default
  al:Auto Locate
  pd:Promote Default
  ogi:Open Game Installation
  vlp:Verify Last Played
  vlo:Verify Last Opened
  ngif:No Game Installation Found
  aid:Add Installation Dialogue
  pi:Parse INI
  cp:Cannot Play
  rtp:Ready to Play
```  
...
- **ParseIni**:
    ...
    - When `HasIni = false`, the window settles to **Cannot Play** with the Launch Game button disabled; the INI tab shows the No INI placeholder.
    - When reading `zoo.ini` or the INI snapshot store fails, the window settles to **Cannot Play** with the Launch Game button disabled; the INI tab shows the
...
A modal hosting a three-column, headered `DataGrid` (`Name`, `Path`, `Status`) and a right-side action panel (`Add`, `Info`, `Edit`, `Delete`, gap, `Fix`).
...
guard runs first if the INI tab has unsaved edits — declining keeps the edits and the view as it is; otherwise the launcher re-verifies the open installation with a pointed  
boot (§7.2.7), so a rename shows, `Cannot Play` becomes `Ready to Play` after a fix, and a deleted installation falls back to the normal resolution (§7.2.4). From **No Game  
Installation Found**, the launcher re-runs the normal boot, so a first installation added via the manager is picked up without requiring a restart. The picker needs nothing  
...
- **`InvalidNoIni`** → three options identical to the **No INI Present** group box ([Section 7.4](#74-ini-config-tab--no-ini-present-state)): `Create Default`, `Locate Manually`,  
  `Copy From Another Installation`. Successful fix writes `Original` and `Current` snapshots.
...
- Never send a second **Save** while one is already in flight — the save path itself, not just the button's disabled state, makes a second request await the in-flight save
  and share its outcome, so a close arriving mid-save can never race it (a save requested while a Revert is in flight fails instead).
...
- **Create Default** → `CreateDefaultIniCommand`. Writes a `zoo.ini` populated from `ZooIniDefaults`' factory values, then runs the standard first-ever parse path (write  
  `Original` + `Current` snapshots, `Source = OriginalImport`). Refreshes the tab into the editor.
- **Locate Manually** → `LocateIniCommand`. Opens a file picker filtered to `*.ini`. On choose, copies the source to the installation's directory as `zoo.ini` and runs first-ever  
  parse.
- **Copy From Another Installation** → `CopyIniFromInstallationCommand`. Opens a selector listing the other registered installations with their names + paths; on confirm, copies  
  the chosen installation's `zoo.ini` to this installation's directory and runs first-ever parse.
...
Modal, read-only mirror of the editor.
...
- Tooltips show `Default: <value>` plus a `Current: <value>` line *unless* the dialogue was opened from the Corrupted INI flow (where `Current` is untrusted; the line is  
  suppressed).
...
The MVP UI uses **Avalonia 11 + `Classic.Avalonia`** for a Windows 95/98 aesthetic. The main window is a single `ClassicWindow` with tabs along the top (General, INI Config).
Modal  
dialogues use `ClassicWindow` too so they get proper title bars (a fix established in the Ref build).  
Cross-cutting UI conventions — token registry (light/dark swatches), standard INI row layout, group-box + icon header pattern, dialogue footer rules, status-bar help wiring,  
theming hooks, icon-name contract — live in [`docs/user-interface-design/conventions.md`](../../user-interface-design/conventions.md). When an agent implements one slice of §9,
the  
...
`docs/user-interface-design/ZooTycoonLauncherHiFiUIPrototype.gif`](../../user-interface-design/ZooTycoonLauncherHiFiUIPrototype.gif) is committed to the repo as a quick reference  
covering every state, every dialogue, the INI tab help affordance, and the dark/light theme switch. The prototype is *not* a production target — match the visual output, not its  
internal structure.
...
- **Window width.** **480 px** while booting (the Looking for Zoo Tycoon state) and **720 px** once booted, driven by `MainWindowViewModel.IsBooting`. The booted width is fixed
  by the INI Config tab's footer button row needing to sit on a single line; height tracks content (`SizeToContent="Height"`). Neither dimension is user-resizable.
- **Menu strip.** Just **File** and **Help** — Edit and View are removed. See §9.10 for the full item list.
- **Tab strip.** Two tabs along the top edge of the content area for the `ReadyToPlay` and `CannotPlay` states: **General** (Alt-G) and **INI Config** (Alt-I). The strip is
...
- **Status bar.** Two cells while booting (primary message + version label); three cells once booted (primary message + secondary detail + version label). `MainWindowViewModel`
  owns the message text and an optional secondary colour brush so the secondary cell can be tinted (e.g. red for `Fix required`).
...
│   └── Dialogues/                         Modal UserControls (Add, Edit, Info, Fix, Historical, Settings, Confirm, Save Changes prompt, error message)  
...
- **Section list (left).** A list of the eight sections in INI order: `[user]`, `[UI]`, `[advanced]`, `[ai]`, `[debug]`, `[language]`, `[Map]`, `[scenario]`.
...
- **Footer row.** Hover / dirty status label on the left (§9.3.2), then `Undo`, `Restore defaults`, `Save` (default), `Revert`. `Save` and `Revert` are disabled when there are
  no pending changes. Button labels carry no ellipses.
...
an optional muted hint sub-line (`px`, `1–60 ticks/sec`, `-10000 silent → 0 full`, etc.). The `[scenario]` section organises its checkboxes by `ScenarioCampaignTier`
(Tutorial → Beginner → Intermediate → Advanced → Expert → Expansion → Unknown).
...
- **Hover/focus the row** → the editor's footer status label switches from the dirty-state indicator to a short italicised one-liner derived from the same prose. Once the cursor  
  leaves the row, the footer reverts to its dirty / saved indicator (`● Unsaved changes` in maroon-bold, or `All changes saved · Last write: <local time>` in muted grey).
...
- **Three-column `DataGrid`:** `Name`, `Path`, `Status` (validity badge).
...
- **Fix Installation** — `ClassicWindow` modal. Two group boxes (`Fix EXE`, `Fix INI`); each has a status indicator (file present / missing), an explanation, and a
  `Locate` / `Create` action button disabled when the file is already present. `OK` default dismisses; the row's validity re-evaluates on close.
...
but with `Open` (default) replacing the row of management actions. There is no separate "switch installation" picker in the MVP; switching is done from the Installation Manager (  
...
- **Snapshot grid.** Three columns — `Captured` (UTC), `Trigger` (`OriginalImport` / `LauncherGui` / `Manual`), `Δ keys` (count). No `Note` column — the prototype's earlier
  "auto-generated note" idea was scoped out (no AI in the launcher).
- **Diff viewer.** A scrolling table with three columns — `Key` (`[section]/keyname`), `Current`, `Snapshot` — one row per key whose value differs between current and the
  selected snapshot. Header text labels the columns so any colour treatment is supplementary, not required. When the diff is empty, the panel shows a centred message: *"No
  differences — this snapshot matches the current state."*
- **Footer:** `Restore` (default; disabled when the diff is empty), `Cancel`.

The diff is computed by enumerating every `(section, key)` present in either snapshot's values and emitting rows where the values disagree (kind-aware comparison — `Bool`
`true` / `1` are equivalent; `NullableInt` empty `==` empty).

### 9.8 Settings dialogue

`ClassicWindow` modal titled `Settings`, opened from `File → Settings`. Three group boxes:

- **Startup** — four-radio group bound to `LauncherStartupPreference`. Labels: `The default installation`, `The installation I last played`,  
  `The installation I last opened in the launcher`, `No installation — show the picker`.
...
Footer: `OK` (default) writes to `LauncherSettings` via `ISettingsService` and closes; `Cancel` discards. The theme change applies live on `OK` (see §9.11) — there is no  
application-level "reload" step.
...
- **System** (default) — follows the OS-level light/dark preference at runtime. Avalonia exposes this via `Application.Current.ActualThemeVariant`; the launcher subscribes to  
  `RequestedThemeVariant` changes and updates the active palette live.
- **Light** — silver-on-navy Win95 palette (the prototype's default), regardless of OS.
- **Dark** — dark-grey-on-navy palette, regardless of OS. The dark palette retains the same bevel topology (white is still "the light bevel highlight"; black is still "the
  deepest  
  bevel edge") but every token is re-pointed so the silver / white / gray / black quartet flips to a coherent Win9x-after-dark scheme. Accent reds, blues, greens, and ambers are  
  re-saturated for legibility on dark surfaces.

Implementation outline (Avalonia):

- A `IThemeService` interface in Application owns the current `LauncherTheme`, exposes a `ThemeVariant Current { get; }`, and emits `ThemeChanged` when the resolved variant  
  changes (whether by user choice or OS-level shift in System mode).
- A `ResourceDictionary`-based `LauncherClassicTheme` in Desktop carries the token set and exposes Light / Dark variants. Classic.Avalonia's own theme is applied first; the  
  launcher's tokens override the bits that need to differ between modes (colour swatches in §9.3 form rows, the maroon / navy diff colouring in §9.7, the muted greys, etc.).
- View-level usage prefers `DynamicResource` over `StaticResource` so swatch swaps propagate without a re-render.
- The prototype's `dk-c-*` utility classes (`dk-c-red`, `dk-c-navy`, `dk-c-maroon`, `dk-c-amber`, `dk-c-green`, `dk-c-dim`) become resource keys (`AccentRed`, `AccentNavy`, etc.)  
  in Avalonia — each pair resolves to a different swatch under the Dark variant.

The Settings dialogue's `OK` calls `IThemeService.SetTheme(LauncherTheme)`; the service writes through `ISettingsService`, recomputes the active variant, raises `ThemeChanged`.  
Views bound through `DynamicResource` repaint immediately. The theme is read once at startup from `LauncherSettings.Theme`.

### 9.12 Prototype reference

A single screen-recording GIF — [`docs/user-interface-design/ZooTycoonLauncherHiFiUIPrototype.gif`](../../user-interface-design/ZooTycoonLauncherHiFiUIPrototype.gif) — captures
every  
state, every dialogue, the INI Config tab help affordance, and the dark/light theme switch in one pass. The recording is the in-repo glance; the **live hi-fi prototype in Claude  
Design** remains the authoritative visual reference (it surfaces hover behaviour, transitions, and the help-on-hover footer affordance that a static frame can't show).
...
- **INI Config tab.** The editor (split layout, eight sections, hover + status help — §9.3.2), the No INI placeholder, Restore Previous INI dialogue with inline diff (  
  §9.7), tooltip parity fix, atomic save with snapshot archiving.
...
- **Settings + theming.** Settings dialogue (§9.8) covers `LauncherStartupPreference`, `CloseAfterGameLaunch`, and `LauncherTheme` (System / Light / Dark — §9.11). Theme applies  
  live via `IThemeService` + `DynamicResource` swatches.
...
- the maintainer's notes at the end of `docs/test-plans/manual-end-to-end-tests.md`.
...
| ID    | Item                                                                                                                                                             | Origin               |
|-------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|
| DW-01 | No-INI recovery actions (§7.4: Create Default, Locate Manually, Copy From Another). The Fix dialogue's INI **Create** button is still hard-disabled.             | PR #5, PR #6         |
| DW-02 | Restore Previous INI dialogue with inline diff (§7.6, §9.7). Still in §13.2's MVP list, but not delivered by the INI Config core slice.                         | INI Config core spec |
| DW-03 | INI editor **Restore defaults** and **Undo** (§7.3.3, §9.3).                                                                                                     | INI Config core spec |
| DW-04 | The `[scenario]` section of the INI Config tab (§5.4, §7.8), after the Phase 0 scenarios spike (§13.1).                                                          | INI Config core spec |
| DW-05 | The General tab keeps its boot-time Cannot Play / "could not read `zoo.ini`" message after a later INI-tab retry succeeds; it should recover without a reopen.   | PR #6 (ruling R10)   |
...
| DW-08 | Installation Info's **History entries** still shows "—" (`InstallationInfoDialogViewModel.cs:44`); count the `Historical` snapshots now that the INI slice stores them. | PR #5 |
| DW-09 | The status bar's display cell is hard-coded as "Display: 1920 × 1080" (`MainWindowViewModel.cs:321`, a `TODO`). Source it from the screen-modes work (§7.9).     | Code `TODO`; maintainer's note 4 |
...
| DW-12 | **MVP, later slice.** Detect a read-only installation folder when an installation opens. Today the first Save simply fails with "access denied". Instead, explain the cause (the folder grants ordinary users read-only access) and recommend the fix: uninstall and reinstall the game, which restores the installer's normal write access. See §14.5 row 9. | Maintainer, 24 Sept 2026 (observed on a real install) |
...
| DW-17 | Unexpected INI tab and editor exceptions reach the user but not the log. `IniConfigTabViewModel` and `IniEditorViewModel` have no logger. The tab's shared in-flight activation also carries the first caller's cancellation token. | INI task review |
| DW-18 | One `IsHelpActive` flag is fed by both hover and focus, so moving the pointer off a row can clear the help while that row still has keyboard focus.            | INI task review      |
...
| ID    | Item                                                                                                                                                             | Origin               |
|-------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|
| DW-20 | The `"Green"` / `"Red"` colour tokens are bare strings across `InstallationValidity`, `ColourTokenToBrushConverter`, the grid and Info; the General tab hard-codes its foreground (`GeneralTabView.axaml:62`, a `TODO`). | PR #4; code `TODO` |
...
| DW-24 | Make spacing, padding and margins consistent across views, and add reusable controls for commonly configured stack and dock panels. `UniformStack` exists, but carries two open `TODO`s (`UniformStack.cs:36-37`). | Maintainer's notes 6–7; code `TODO` |
...
| DW-27 | Application and Desktop INI hygiene: `IniErrors.StoreFailed` says "could not be opened" but also covers reconcile and commit failures; the `ApplicationServiceCollectionExtensions` summary is stale; the `[ai]` footnote wording drifts from the INI spec; the language rows re-spell their ids instead of using the constants; `IniToggleFieldViewModel.IsTrue` duplicates `IniKeySpec.TryParseBool`; the language picker raises `SelectedOption` between its two writes, and its designer constructor loads no rows; `IniConfigTabViewModel.Content` has a public setter; the `ActivateAsync` doc omits the no-mediator skip; the `IDialogService.ShowErrorAsync` doc omits the ownerless, non-modal fallback. | INI task review |
...
| ID    | Item                                                                                                                                                             | Origin               |
|-------|------------------------------------------------------------------------------------------------------------------------------------------------------------------|----------------------|
| DW-35 | Run the manual end-to-end INI Config Core stubs 1–8. Smoke checks:<br>• row help appears on hover and on focus (task ruling R3);<br>• combo-box selections survive section switches;<br>• an empty text field keeps its width;<br>• Escape and Enter work on the save prompt;<br>• re-activating the tab after a game session picks up the game's writes;<br>• footer help follows keyboard section changes. | PR #6 |
...
| 9 | Read-only installation folder               | Risk (confirmed) | Observed 24 Sept 2026. One install's folder gave ordinary users read-only access, so every Save failed with "access denied" (the launcher and Zookini alike; only admin rights worked). Reinstalling restored the installer's normal write access, after which saves succeeded. MVP: detect and explain, recommending a reinstall (DW-12). V2.0.0: a VirtualStore copy as the fix, after proving the game reads it (DW-36). |  
